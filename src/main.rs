...
// get everything in JSON https://cloud.timeedit.net/liu/web/schema/objects//o.json?fr=t gives JSON 
...
fn App() -> Element {
    let fetch = use_resource(|| async {
        ...
        let response = reqwest::get(format!("https://cloud.timeedit.net/liu/web/schema/objects//o.json?fr=f&types=205&sid=3")).await.unwrap().text().await.unwrap();
        response
    });
